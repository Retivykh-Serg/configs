    ...
    while True:
        try:
            ...
            # Block until lock is available, then execute.
                ...
                logger.info(
                    "Waited %s seconds to acquire lock. Executing command: %s" % (duration.seconds, cmd,)
                )
...
def main():
    # Parse arguments.
    parser = argparse.ArgumentParser(description="Attempt to acquire Redis lock and execute command.",)
    parser.add_argument(
        "cmd", help="command to execute when lock has been acquired", nargs=argparse.REMAINDER,
    )
    parser.add_argument(
        "-b", "--block", action="store_true", help="block until lock is acquired",
    )
    parser.add_argument(
        "--redis-host", default=REDIS_HOST, help="host of redis server (default: %s)" % REDIS_HOST,
    )
    parser.add_argument(
        "--redis-port", default=REDIS_PORT, help="port of redis server (default: %s)" % REDIS_PORT,
    )
    group = parser.add_mutually_exclusive_group()
    group.add_argument(
        "-q", "--quiet", action="store_true", help="silence standard output",
    )
    group.add_argument(
        "-v",
        "--verbose",
